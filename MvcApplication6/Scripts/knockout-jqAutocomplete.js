...
;(function(factory) {
        ...
        this.init = function(element, valueAccessor, allBindings, data, context) {
            ...
            if (typeof options.source === "function" && !ko.isObservable(options.source)) {
                config.source = function(request, response) {
                    ...
                    //call the provided function for retrieving dataoptions.tablica +
                    options.source.call(context.$data, { tablica: options.options.tablica(), pojam: request.term }, callback);
                };
            }
            else {
                //process local data
    ...
    ko.bindingHandlers.jqAutoComplete = new JqAuto();
});
