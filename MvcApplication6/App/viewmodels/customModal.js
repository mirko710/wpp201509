define(['plugins/dialog', 'services/dataService'], function (dialog, data) {
    ...
    var CustomModal = function (title) {
        this.title = title;
        this.init = init;
    ...
    function provjeriMjere() {
        ...
        if (valid) {
            ...
            //redakUpita()[rowOpenDialog()].vrijednost4(V4);
            ...
            //realFakeMjere.push(fejk);
            opened = false;
            dialog.close(this, { r1: realFakeMjere()[0], vrijednost1: V1, vrijednost4: V4 });
   ...
   var mapFejkRow = function (nesto) {
        var uOperator = nesto.redakOperator;
        var ind = MjereOperators.indexOf(uOperator);
        var fejk=null;
        realFakeMjere([]);
        if (nesto.redakUpit) {
            //realFakeMjere.push(nesto.redakUpit);
            fejk = nesto.redakUpit;
            //realFakeMjere()[0]['MJR_Operator'](ind);
        } else {
            fejk = new fakeMjere();
        }
    ...
    CustomModal.prototype.show = function (nesto) {
        opened = true;
        mapFejkRow(nesto);
